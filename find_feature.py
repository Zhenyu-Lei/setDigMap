...
from arrayDifference import calculate_gray_sum
...
def process_image2(image_path, down_sampling=4, k=20, L=15, pix_thresholded=0.9):
    if os.path.exists(f'processed_image_{down_sampling}_{k}_{L}_{pix_thresholded}.png'):
        print("processed_image.jpg has exist")
        return cv2.imread(f'processed_image_{down_sampling}_{k}_{L}_{pix_thresholded}.png', cv2.IMREAD_GRAYSCALE)
    else:
        ...
        sum_of_y, sum_of_x = peak_process_image(thresholded_image, True)
        ...
        print("field count: {}".format(len(peak_range_list)))
        cnt = 0
        # 由于图象可能会有像素级缺损，给定一个满足条件的阈值 pix_thresholded
...
def process_image3(image_path, down_sampling=4, k=20, L=15, pix_thresholded=0.9):
    if os.path.exists(f'processed_image_{down_sampling}_{k}_{L}_{pix_thresholded}.png'):
        print("processed_image.jpg has exist")
        return cv2.imread(f'processed_image_{down_sampling}_{k}_{L}_{pix_thresholded}.png', cv2.IMREAD_GRAYSCALE)
    else:
        ...
        sum_of_y, sum_of_x = calculate_gray_sum(thresholded_image)
        height, width = thresholded_image.shape
        y_peak_ranges, x_peak_ranges = find_peak_ranges(sum_of_y[height - 1, :], sum_of_x[:, width - 1])
        peak_range_list = generate_peak_ranges(x_peak_ranges, y_peak_ranges)
        print(len(peak_range_list))
